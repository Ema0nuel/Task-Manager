(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))o(s);new MutationObserver(s=>{for(const n of s)if(n.type==="childList")for(const r of n.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&o(r)}).observe(document,{childList:!0,subtree:!0});function a(s){const n={};return s.integrity&&(n.integrity=s.integrity),s.referrerPolicy&&(n.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?n.credentials="include":s.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function o(s){if(s.ep)return;s.ep=!0;const n=a(s);fetch(s.href,n)}})();function l(e){window.scrollTo({top:0,left:0,behavior:"smooth"}),document.title=`${e}`}const u=()=>({html:`
    ...
    `,pageEvents:t}};function p(e){const t=JSON.parse(localStorage.getItem("tasks"))||[];e.addEventListener("submit",a=>{a.preventDefault();let s=document.getElementById("task-title").value.trim();s&&(t.push({id:crypto.randomUUID(),taskName:s,completed:!1}),localStorage.setItem("tasks",JSON.stringify(t)),i("task"))})}function m(e){const t=JSON.parse(localStorage.getItem("tasks"))||[];e.addEventListener("click",()=>{const a=e.getAttribute("data-task");t.forEach(o=>{if(a===o.id){let s=t.filter(n=>n.id!==a);localStorage.setItem("tasks",JSON.stringify(s)),i("task")}})})}function g(e){const t=JSON.parse(localStorage.getItem("tasks"))||[];e.addEventListener("click",()=>{const a=e.getAttribute("data-task");t.forEach(async o=>{a===o.id&&await i("taskDetail",a)})})}function x(e,t){e.addEventListener("click",()=>{const a=e.getAttribute("data-task"),o=t.find(s=>s.id===a);if(o){const s={...o,completed:!0},n=t.map(r=>r.id===a?s:r);localStorage.setItem("tasks",JSON.stringify(n)),i("task")}})}function b(){const e=JSON.parse(localStorage.getItem("tasks"));let t="";return e&&e.length!==0?e.forEach(a=>{t+=`
            ...
            `}):t='<div class="text-center">No Task!</div>',t}const h=()=>{l("Task");const{html:e}=u();function t(){const a=document.getElementById("task-form"),o=document.querySelectorAll(".delete-btn"),s=document.querySelectorAll(".edit-btn");a&&p(a),o&&o.forEach(n=>{m(n)}),s&&s.forEach(n=>{g(n)})}return{html:`
